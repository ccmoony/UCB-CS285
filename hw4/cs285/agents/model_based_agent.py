...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        obs_deltas_normalized = (obs_deltas - self.obs_delta_mean) / (self.obs_delta_std + 1e-8)
        inputs = torch.cat([obs, acs], dim=1)
        inputs_normalized = (inputs - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        loss = self.loss_fn(self.dynamics_models[i](inputs_normalized), obs_deltas_normalized)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        self.obs_acs_mean = inputs.mean(dim=0)
        self.obs_acs_std = inputs.std(dim=0)
    ...
    def get_dynamics_predictions(
        ...
        inputs = torch.cat([obs, acs], dim=1)
        inputs_normalized = (inputs - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        obs_delta = self.dynamics_models[i](inputs_normalized)
        obs_delta = obs_delta * (self.obs_delta_std + 1e-8) + self.obs_delta_mean
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        for acs in action_sequences.transpose(1, 0 ,2):
            ...
            rewards = self.env.get_reward(next_obs.reshape(-1, self.ob_dim), acs.reshape(-1, self.ac_dim))[0].reshape(self.ensemble_size, self.mpc_num_action_sequences)
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            for i in range(self.cem_num_iters):
                ...
                rewards = self.evaluate_action_sequences(obs, action_sequences)
                elite_indices = np.argsort(rewards)[::-1][:self.cem_num_elites]
                elite_actions = action_sequences[elite_indices]
                elite_mean_new, elite_std_new = elite_actions.mean(axis=0), elite_actions.std(axis=0)
                elite_mean, elite_std = self.cem_alpha * elite_mean_new + (1 - self.cem_alpha) * elite_mean, self.cem_alpha * elite_std_new + (1 - self.cem_alpha) * elite_std
            return elite_mean[0]
        else:
